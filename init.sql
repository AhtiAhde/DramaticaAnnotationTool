...
CREATE TABLE users (id UUID PRIMARY KEY, email TEXT UNIQUE, pwd TEXT);
CREATE TABLE characters (id SERIAL PRIMARY KEY, book_id INTEGER NOT NULL, name TEXT NOT NULL);
CREATE TABLE role_annotations (id SERIAL PRIMARY KEY, user_id UUID NOT NULL, char_id INTEGER NULL, role TEXT NOT NULL);
...
CREATE TABLE annotation_arc (id SERIAL PRIMARY KEY, book_id INTEGER, user_id UUID, title TEXT, short_desc TEXT);
CREATE TABLE meta_aux (id SERIAL PRIMARY KEY, user_id UUID, paragraph_id INTEGER, aux_type aux_type);
